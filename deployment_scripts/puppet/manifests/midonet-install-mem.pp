...
$ana_public_ip_hash     = get_node_to_ipaddr_map_by_network_role($nsdb_hash, 'public')
$ana_public_ip_list     = values($ana_public_ip_hash)
$ana_keys               = keys($ana_hash)


$ana_public_ip          = empty($ana_keys)? {true => $public_vip , default => $ana_public_ip_list[0] }

$ssl_horizon            = hiera_hash('public_ssl')['horizon']
...
if ( $ssl_horizon ) or ($midonet_settings['mem_override_servername'] == true and $midonet_settings['mem_override_ssl_key'] != '' and $midonet_settings['mem_override_ssl_certificate'] != '' ) {
...
file { 'mem-vhost':
  ...
  path    => '/etc/apache2/sites-available/30-midonet-mem.conf.conf',
  content => template('/etc/fuel/plugins/midonet-4.0/puppet/templates/midonet-mem.conf.erb'),
}

exec { "enable mem-vhost":
    path    => "/usr/bin:/usr/sbin:/bin",
...
## It is not needed to supply the certificate here since
## it will be already using the Horizon configuration on Haproxy
