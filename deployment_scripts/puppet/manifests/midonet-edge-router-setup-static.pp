...
$static_linux_bridge_ip_address      = split($static_linux_bridge_ip_netl,'/')[0]
$static_fake_edge_router_ip_address  = split($static_fake_edge_router_ip_netl,'/')[0]
...
neutron_subnet { 'edge-subnet':
  ...
  enable_dhcp  => false,
  cidr         => generate_cidr_from_ip_netlength($static_linux_bridge_address),
  tenant_id    => $external_net['tenant'],
  network_name => 'ext-net',
} ->

neutron_port { 'port-static':
  ...
  ip_address      => [[$static_fake_edge_router_address]],
} ->
