...
if($old_config != 'notfound')
{
  ...
  if ($old_gw_type == 'static') {
    ...
    file { 'cleanup static script':
      ...
      content => template('/etc/fuel/plugins/midonet-5.0/puppet/templates/cleanup_static_gateway.sh.erb'),
    }
    ...
    file { 'delete router interfaces script':
      ...
      content => template('/etc/fuel/plugins/midonet-5.0/puppet/templates/remove_router_interfaces.sh.erb'),
    }
